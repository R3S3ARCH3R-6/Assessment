...
public class CustomerDAOImpl implements CustomerDAO {
	...
	public int updateCustomer(int id, long contact) throws BusinessException {
		//this is the same code found in "CustomerServiceImpl.java" 
		int c = 0;
	...
	//this is done by Wesley
		//I wonder if I should be using the ArrayList and only return the correct return type 
	@Override
	public Customer getCustomerById(int id) throws BusinessException {
		Customer customer = null;
		try(Connection connection=MySqlConnection.getConnection()){
			customer = new Customer();
			String sql="select id,name,city,age,contact from customer where id=?";
			PreparedStatement preparedStatement=connection.prepareStatement(sql);
			preparedStatement.setInt(1, id);
			ResultSet resultSet=preparedStatement.executeQuery();
			if(resultSet.next()) {
				...
				throw new BusinessException("No customers with ID " + id); 
			}
	...
	public Customer getCustomerByContact(long contact) throws BusinessException {
		...
		try(Connection connection=MySqlConnection.getConnection()){
			customer = new Customer();
			String sql="select id,name,city,age,contact from customer where contact=?";
			PreparedStatement preparedStatement=connection.prepareStatement(sql);
			preparedStatement.setLong(1, contact);
			ResultSet resultSet=preparedStatement.executeQuery();
			if(resultSet.next()) {
				...
				throw new BusinessException("No customers with contact " + contact); 
			}
	...
	public List<Customer> getCustomersByAge(int age) throws BusinessException {
		List<Customer> customersList=new ArrayList<>();
		try(Connection connection=MySqlConnection.getConnection()){
			String sql="select id,name,city,age,contact from customer where city=?";
			PreparedStatement preparedStatement=connection.prepareStatement(sql);
	...
	public List<Customer> getCustomersByName(String name) throws BusinessException {
		List<Customer> customersList=new ArrayList<>();
		try(Connection connection=MySqlConnection.getConnection()){
			String sql="select id,name,city,age,contact from customer where name=?";
			PreparedStatement preparedStatement=connection.prepareStatement(sql);
